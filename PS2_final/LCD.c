...
void display_hex(int hex,char int_enable, char line, char position)
{
	int show_i=0;
//	char hex_part[2]= {0};
...
/*	if(hex>255)
	{
		hex_part[2]= (hex& 0xFF00)>>8
		int_enable=1;
	}
*/
	for(hex_i=(int_enable*2+2); hex_i>0; hex_i--)
	{
		show_i = (hex >> ((hex_i-1)*4)) & 0x0F;

		if(show_i>9)
		{
			char_enable=1;
		}
		data(show_i+48+(char_enable*7));
		char_enable=0;
	}
}
